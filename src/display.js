...
import { fetchImages } from './search.js';
import { makeElementDraggable } from './drag.js';

const settingsFilePath = path.join(__dirname, './settings.json');
let selectedImage = null; // グローバルスコープで宣言

const downloadImage = async (url, filepath) => {
  const response = await fetch(url);
  const buffer = Buffer.from(await response.arrayBuffer());
  await ipcRenderer.invoke('write-file', filepath, buffer);
...
export const displayImages = async (query) => {
...
export const showImagesFromDirectory = async () => {
...
export const saveImagePositions = async (imageSize) => {
  const images = document.querySelectorAll('.draggable');
  const positions = {};
...
export const loadImagePositions = () => {
  showImagesFromDirectory();
};

// 選択された画像のパスを取得する関数
export const getSelectedImagePath = () => {
  if (selectedImage) {
